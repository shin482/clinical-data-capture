...
export async function GET(request: NextRequest) {
  ...
  const valueRows = db().prepare(`SELECT s.subject_id, v.timepoint, c.variable_key, c.value, c.missing_reason FROM clinical_values c JOIN subjects s ON s.id=c.subject_id JOIN visits v ON v.id=c.visit_id ${subjectId ? 'WHERE s.subject_id=?' : ''}`).all(...(subjectId ? [subjectId] : [])) as { subject_id: string; timepoint: string; variable_key: string; value: string; missing_reason: string | null }[]
  const data = subjects.map((subject) => { const row: Record<string, string> = { SUBJECT_ID: subject.subject_id }; valueRows.filter((value) => value.subject_id === subject.subject_id).forEach((value) => { row[`${value.variable_key}_${value.timepoint.toLowerCase()}`] = value.value || ''; if (value.missing_reason) row[`${value.variable_key}_${value.timepoint.toLowerCase()}_missing_reason`] = value.missing_reason }); return row })
