...
async function main() {
  ...
  for (const key of derivedRules[0].sources) await save(key, '1')
  ...
  assert.equal(rows[1][expected.indexOf('crp_t1')], '')
  console.log('PASS: conditional rules, 14 lab children, smoking 0/1/99/blank, retained child values, SINBAD recomputation/persistence/read-only/missing/99, complete ordered export including empty database')
  db().close()
}
